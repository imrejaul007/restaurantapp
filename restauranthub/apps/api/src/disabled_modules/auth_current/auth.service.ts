...
export class AuthService {
  ...
  async signIn(signInDto: SignInDto) {
    ...
    if (user) {
      this.logger.debug(`User status check - Active: ${user.isActive}, Verified: ${user.isVerified}`);
    }
  ...
  async forgotPassword(forgotPasswordDto: ForgotPasswordDto) {
    ...
    // Generate reset token with dedicated secret for enhanced security
    const resetToken = this.jwtService.sign(
      { sub: user.id, email: user.email, type: 'password-reset' },
      {
        secret: this.configService.get('JWT_PASSWORD_RESET_SECRET') || this.configService.get('JWT_SECRET'),
        expiresIn: '1h',
      },
    );
    ...
    // Send reset email
    await this.sendPasswordResetEmail(user.email, resetToken, 'User');
  ...
  private async sendVerificationEmail(email: string, firstName: string) {
    const verificationToken = this.jwtService.sign(
      ...
      {
        secret: this.configService.get('JWT_EMAIL_VERIFICATION_SECRET') || this.configService.get('JWT_SECRET'),
        expiresIn: '24h',
      },
    );

...
export class BruteForceGuard implements CanActivate {
  ...
  private async checkIPBruteForce(ip: string, userAgent: string, path: string): Promise<boolean> {
    ...
    try {
      ...
      if (recentAttempts >= maxAttempts) {
        // Block the IP - temporarily disabled due to schema issues
        // await this.prisma.auditLog.create({
        //   data: {
        //     userId: 'system', // Use system user for IP blocks
        //     action: 'IP_BLOCKED',
        ...
        this.logger.warn(`IP ${ip} would be blocked due to ${recentAttempts} failed attempts (audit logging temporarily disabled)`);
        return true;
      }
  ...
  private async checkAccountBruteForce(email: string, ip: string, userAgent: string, path: string): Promise<boolean> {
    ...
    try {
      ...
      if (recentAttempts >= maxAttempts) {
        // Lock the account - temporarily disabled due to schema issues
        // await this.prisma.auditLog.create({
        //   data: {
        //     userId: 'system', // Use system user for account locks
        //     action: 'ACCOUNT_LOCKED',
        ...
        this.logger.warn(`Account ${email} would be locked due to ${recentAttempts} failed attempts (audit logging temporarily disabled)`);
        return true;
      }
  ...
  async recordFailedAttempt(email: string, ip: string, userAgent: string, reason: string): Promise<void> {
    try {
      ...
      // await this.prisma.auditLog.create({
      //   data: {
      //     userId: 'system', // Use system user for failed login attempts
      //     action: 'FAILED_LOGIN',
  ...
  async recordSuccessfulAttempt(email: string, ip: string, userAgent: string): Promise<void> {
    try {
      ...
      // await this.prisma.auditLog.create({
      //   data: {
      //     userId: 'system', // Use system user for successful login attempts

...
export class SecureTokenService {
  ...
  async storePasswordResetToken(userId: string, token: string): Promise<void> {
    try {
      ...
      this.logger.warn(`Redis unavailable, using database fallback for password reset token: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  async validatePasswordResetToken(userId: string, token: string): Promise<boolean> {
    try {
      ...
      this.logger.warn(`Redis unavailable, using database fallback for token validation: ${error instanceof Error ? error.message : String(error)}`);
    }

    // Database fallback - validate against stored hash
    try {
      // Temporarily disabled until resetToken fields are available
      ...
      // if (!user?.resetTokenHash || !user?.resetTokenExpiresAt) {
        this.logger.debug(`No password reset token found in database for user ${userId} - feature temporarily disabled`);
        return false;
      // }
  ...
  async deletePasswordResetToken(userId: string): Promise<void> {
    try {
      ...
      this.logger.warn(`Failed to delete password reset token from Redis: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    try {
      ...
      //   },
      // });
      this.logger.log(`Password reset token deletion temporarily disabled for user ${userId}`);
    } catch (error) {

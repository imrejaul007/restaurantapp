import { PrismaClient, UserRole, RestaurantCategory, VerificationStatus } from '@prisma/client';
...
async function main() {
  ...
  const employee = await prisma.employee.create({
    data: {
      ...
      skills: ['Cooking', 'Customer Service', 'Team Leadership', 'Food Presentation'],
  ...
  await Promise.all([
    prisma.vendorOffering.create({
      data: {
        ...
        images: ['/images/vegetables.jpg', '/images/fresh-produce.jpg'],
        isFeatured: true,
        isActive: true,
      },
    }),
  ...
  const job1 = await prisma.job.create({
    data: {
      ...
      requirements: ['3+ years cooking experience', 'Knowledge of Indian cuisine', 'Food safety certification'],
  ...
  const job2 = await prisma.job.create({
    data: {
      ...
      location: 'Mumbai, Maharashtra',
      requirements: ['Good communication skills', 'Customer service experience preferred', 'Flexible working hours'],
      benefits: ['Tips', 'Staff meals', 'Growth opportunities'],
      status: 'open',
      isPremium: false,
      expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days
      createdBy: restaurantUser.id,
